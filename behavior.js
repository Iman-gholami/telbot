...
const BANTER = /(😂|🤣|خخخ|ههه|کل.?کل|باز شروع|چی میگی|نه بابا|گیر دادی|برو بابا|زر نزن|اسکل|خل|دیوونه|مسخره|گوه|چرت|احمق|مغزت)/;
...
const SOCIAL_QUESTION = /(چرا\s+(?:اینقد|انقد|باید\s+تو)|مغزت|خوبی|چطوری|کجایی|دوستم داری|دوستش دارم|چی میگی|داری چیکار میکنی|داری چیکار می‌کنی)/;
...
export function fixedFallback({ quotaExhausted = false } = {}) {
  if (quotaExhausted) {
    return pick([
      "الان مدل رایگان جواب نمی‌ده؛ یه کم بعد دوباره بگو 😅",
      "فعلاً هوش آنلاینم گیر کرده، ولی خودم هستم 😌",
      "مدل رایگان فعلاً قاطی کرده؛ یکم بعد دوباره امتحان کن.",
    ]);
  }
  return pick([
    "این یکی از دستم در رفت، دوباره بگو 😅",
    "یه لحظه قاطی کردم، دوباره بگو چی گفتی.",
