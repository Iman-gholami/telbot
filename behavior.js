...
const BANTER = /(😂|🤣|خخخ|ههه|کل.?کل|باز شروع|چی میگی|نه بابا|گیر دادی|برو بابا|زر نزن|اسکل|خل|دیوونه|مسخره|گوه|چرت)/;
const FACTUAL = /(چطور|چجوری|چرا|چیست|چیه|کیه|کجاست|چند|قیمت|خبر|امروز|الان|آخرین|جدیدترین|آپدیت|نسخه|ارور|خطا|کد|برنامه نویسی|پزشکی|دارو|تاریخ|ساعت|هوا|سرچ|جستجو|لینک|منبع)/;

export function isFactualIntent(text) {
  const n = normalizePersian(text);
  return /[؟?]/.test(n) || FACTUAL.test(n);
}
...
export function fixedFallback({ quotaExhausted = false } = {}) {
  if (quotaExhausted) {
    return pick([
      "امروز سهم هوشم تموم شد 😵‍💫 یه کم بعد دوباره امتحان کن.",
      "فعلاً سهمیه‌م ته کشیده؛ بعداً دوباره صدام کن 😌",
    ]);
  }
  return pick([
    "جانم؟ الان مغزم یه لحظه قفل کرد، دوباره بگو 😅",
    "هستم؛ این یکی از دستم در رفت، یه بار دیگه بپرس.",
    "یه لحظه قاطی کردم 🙈 دوباره بگو چی می‌خواستی.",
  ]);
}
